...
def random_sample(replication, batch_size, weight, noise_scale):
    rng = np.random.default_rng(42)
    while True:
        idx = rng.choice(X_full.shape[0], replication*batch_size, p=weight)
        X_batch, Y_batch = X_full[idx]/255., Y_full[idx]
        X_batch = X_batch.reshape(replication, batch_size, *X_full.shape[1:])
        Y_batch = Y_batch.reshape(replication, batch_size, *Y_full.shape[1:])
        noise = rng.normal(0, noise_scale, X_batch.shape)
        X_batch = np.clip(X_batch + noise, 0, 1)
        yield jnp.array(X_batch), jnp.array(Y_batch)


def full_pass(replication, batch_size, noise_scale):
    rng = np.random.default_rng(42)
...
if __name__ == '__main__':
    ...
    train_loader = random_sample(replication, batch_size, weight, noise_scale)
    X_batch, Y_batch = next(train_loader)
    assert X_batch.shape == (replication, batch_size, 64, 64, 3)
    assert Y_batch.shape == (replication, batch_size, 6)

    test_loader = full_pass(replication, batch_size, noise_scale)
